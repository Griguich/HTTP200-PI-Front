...
this.translate.use(lang);
  ...
  ngOnInit(): void {
   ...
   addComment(idPub: number) {
    ...
    this.commentaireService.assignCommentaireToPub(this.newCommentaire, idPub, userId).subscribe(
      comment => {
 ...
 /* onAddPublication(publication: Publication): void {
  ...
  /*onFileSelected(event: any) {
    ...
    toggleLike(idPub: number, likeTypeStr: string) {
      ...
      this.likeService.toggleLikesP(idPub, likeType, idUser).subscribe(
        (like: Like) => {
    ...
    toggleLikeC(idComm: number, likeTypeStr: string) {
      ...
      const idUser=1;
      // Call the toggleLikesP method from the LikeService
      this.likeService.toggleLikesC(idComm, likeType, idUser).subscribe(
        (like: Like) => {
          console.log(`Like status updated: ${like.likeType}`);
        },
        (error: any) => {
    ...
    toggleFavorite( idPub: number) {
      const idUser =1;
      this.publicationService.toggleFavoritePublication(idUser, idPub).subscribe(
    ...
    getFavorites() {
      ...
      this.publicationService.getFavoritePublicationsByUserId(idUser).subscribe(
        (publications: Publication[]) => {
